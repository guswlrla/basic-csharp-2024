...
namespace NewBookRentalShopApp
{
    public partial class FrmBookRental : MetroForm
    {
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            // 반납일이 1800-01-01이면 반납일에 null을 입력
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
                {
                    ...
                    if (DtpReturnDate.Value <= DtpRentalDate.Value) // 대출일보다 반납일이 뒷날짜이여야 함
                    {
                        returnDate = "";
                    }
                    else
                    ...
                    if (result > 0)
                    {
                        // this : 메시지박스의 부모창이 누구인지 봄(FrmLoginUser)
        ...
        private void RefreshData()
        {
            using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
            {
                ...
                adapter.Fill(ds, "rentaltbl"); // 대표테이블 이름 사용
        ...
        private void DgvResult_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1) // 아무것도 선택하지 않으면 -1
            {
                ...
                DtpReturnDate.Value = !string.IsNullOrEmpty(selData.Cells[6].Value.ToString()) ? 
                                  ...
                                  DateTime.Parse("1800-01-01"); // 1800-01-01은 반납 안한 것
