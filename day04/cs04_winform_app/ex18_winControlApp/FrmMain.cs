...
namespace ex18_winControlApp
{
    public partial class FrmMain : Form
    {
        ...
        public FrmMain()
        {
            ...
            GrbEditor.Text = "텍스트에디터"; // 코드비하인드 디자인셋팅, 이렇게 하지 말 것!
        }

        private void FrmMain_Load(object sender, EventArgs e)
        {
            var Fonts = FontFamily.Families; // 현재 OS 내에 설치된 폰트를 다 가져옴
        ...
        // 글자체, 볼드체, 이탤릭으로 변경하는 메서드
        void ChangeFont()
        {
            if (CboFonts.SelectedIndex < 0) // 아무것도 선택안됨
                return;
            FontStyle style = FontStyle.Regular; // 일반글자(볼드x, 이탤릭x)로 초기화

            if (ChkBold.Checked == true)  // "굵게" 체크박스를 체크하면
                style |= FontStyle.Bold;

            if (ChkItalic.Checked == true) // "이탤릭" 체크박스를 체크하면
        ...
        {
            var res = MessageBox.Show("집에 가고 싶나요?", "질문", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void BtnFileLoad_Click(object sender, EventArgs e)
        {
            ...
            {
                // UTF-8이 한글이 깨짐, EUC-KR(Window 949), UTF-8(BOM)은 문제 x
        ...
        private void DoRealWork(BackgroundWorker worker, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < maxValue; i++)
            {
                ...
                else
                {
                    ...
                    // 아래를 실행하면 BgwProgress_ProgressChanged 이벤트핸들어의
